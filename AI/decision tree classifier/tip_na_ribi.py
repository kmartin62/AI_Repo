...
if __name__ == '__main__':
    ...
    dataset_x = list()
    for d in dataset:
        value = [d[i] for i in range(len(d)) if i != col_index]
        dataset_x.append(value)

    train_set = dataset_x[:int(0.85 * len(dataset_x))]
    test_set = dataset_x[int(0.85 * len(dataset_x)):]

    X = [train_set[i][:-1] for i in range(len(train_set))]
    Y = [train_set[i][-1] for i in range(len(train_set))]

    clf = RandomForestClassifier(n_estimators=n_samples, criterion=criterion, random_state=0)
    clf.fit(X, Y)
    ...
    print(couter / len(test_set))

    entry_x = [entry[i] for i in range(len(entry)) if i != col_index]
    print(clf.predict([entry_x])[0])
    print(clf.predict_proba([entry_x]))
